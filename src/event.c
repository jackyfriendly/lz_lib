...
__event_getindex (struct event_pool *event_pool, int fd, int idx)
{
	...
	if (event_pool == NULL) {
		Lz_log ("event", LZ_LOG_ERROR, "invalid argument");
		return -1;
	}
...
event_pool_new_epoll (int count)
{
	...
	if (epfd == -1) {
		Lz_log ("epoll", LZ_LOG_ERROR, "epoll fd creation failed (%s)",
			strerror (errno));
		free (event_pool->reg);
		free (event_pool);
		return NULL;
