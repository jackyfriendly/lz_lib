...
#define LZ_LOG_ERRNO(errno) ((errno == ENOTCONN) ? LZ_LOG_DEBUG : LZ_LOG_ERROR)
...
//todo: notify this
//    xlator_notify (this->xl, LZ_EVENT_POLLERR, this);
...
socket_event_poll_out (transport_t *this)
{
    ...
    pthread_mutex_unlock (&priv->lock);
//todo: notify this
//    xlator_notify (this->xl, LZ_EVENT_POLLOUT, this);
        ...
        size_t *size1_p, size_t *size2_p)
{
    ...
    if (size1 <= 0 || size1 > 1048576) {
        lz_log (__func__, LZ_LOG_DEBUG,
                "socket header has incorrect size1=%"LZ_PRI_SIZET,
                size1);
        return -1;
    }
    ...
    if (size2 > (131072)) {
        lz_log (__func__, LZ_LOG_DEBUG,
                "socket header has incorrect size2=%"LZ_PRI_SIZET,
                size2);
        return -1;
    }
        ...
        switch (priv->incoming.state) {
            ...
            case SOCKET_PROTO_STATE_HEADER_CAME:
                ...
                if (size2) {
                    ...
                    */
                    iobuf = iobuf_get (this->iobuf_pool);
                    if (!iobuf) {
                        lz_log (__func__, LZ_LOG_ERROR,
...
socket_init (transport_t *this)
{
    ...
    if (!priv) {
        lz_log (__func__, LZ_LOG_ERROR,
                "calloc (1, %"LZ_PRI_SIZET") returned NULL",
                sizeof (*priv));
        return -1;
    }
...
init (transport_t *this)
{
    ...
    if (ret == -1) {
        lz_log (__func__, LZ_LOG_DEBUG, "socket_init() failed");
    }
