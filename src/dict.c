	   ...
	   data_t *value)
{
	...
	if (!key) {
		ret = asprintf (&key, "ref:%p", value);
                if (-1 == ret) {
                        lz_log ("dict", LZ_LOG_ERROR, "asprintf failed");
                        return -1;
                }
		key_free = 1;
	}
		    ...
		    dict_t *new)
{
	...
	LZ_VALIDATE_OR_GOTO("dict", dict, fail);

	if (new == NULL) {
		local_new = dict_new ();
		LZ_VALIDATE_OR_GOTO("dict", local_new, fail);
		new = local_new;
	}
...
_dict_serialized_length (dict_t *this)
{
	...
	if (count < 0) {
		lz_log ("dict", LZ_LOG_ERROR, "count (%d) < 0!", count);
		goto out;
	}
	...
	while (count) {
		...
		if (!pair->key) {
			lz_log ("dict", LZ_LOG_ERROR, "pair->key is null!");
			goto out;
		}
...
dict_serialized_length (dict_t *this)
{
	...
	if (!this) {
		lz_log ("dict", LZ_LOG_ERROR, "this is null!");
		goto out;
	}
	...
	if (!buf) {
        ...
        { 
                ...
                *buf = CALLOC (1, len);
                if (*buf == NULL) {
                        ret = -ENOMEM;
                        lz_log ("dict", LZ_LOG_ERROR, "out of memory");
                        goto unlock;
                }
